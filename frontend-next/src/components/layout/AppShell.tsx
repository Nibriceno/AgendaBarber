...
import { getRoleAreaLabel } from "@/features/auth/lib/auth-routing";
...
export default function AppShell({ children, businessSlug }: AppShellProps) {
  ...
  const navigation = APP_NAVIGATION.filter(
    (item) =>
      item.roles.includes(user.role) &&
      (!user.billingRestricted || item.segment === "subscription"),
  );

  const publicHomeHref = user.billingRestricted
    ? `/${businessSlug}/subscription`
    : `/${businessSlug}`;

  const areaLabel = getRoleAreaLabel(user.role);
  ...
  const currentPage =
    navigation.find((item) => {
      const href = `/${businessSlug}/${item.segment}`;
    ...
    <div className="min-h-screen bg-zinc-50">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-zinc-200 bg-white lg:flex lg:flex-col">
        <div className="flex h-20 items-center border-b border-zinc-100 px-6">
          <Link href={publicHomeHref} className="flex items-center gap-3">
          ...
          <aside className="relative z-50 flex h-full w-72 flex-col bg-white shadow-xl">
            <div className="flex h-20 items-center justify-between border-b border-zinc-100 px-5">
              ...
              >
                ...
                <span className="font-semibold">{PLATFORM_BRAND_NAME}</span>
              </Link>

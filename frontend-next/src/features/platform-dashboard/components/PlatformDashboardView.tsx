  ...
  return (
    <div className="space-y-7">
      <section className="relative overflow-hidden rounded-3xl bg-white p-6 shadow-sm ring-1 ring-zinc-200 sm:p-8">
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-500">El {operationalPercentage}% de las barberías registradas se encuentra operativo. Aquí tienes el estado actualizado de {PLATFORM_BRAND_NAME}.</p>
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="/super-admin/businesses?create=1" className="inline-flex h-11 items-center rounded-xl bg-zinc-950 px-4 text-sm font-semibold text-white transition hover:bg-zinc-800">Nueva barbería</Link>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Métricas de plataforma">
        <MetricCard label="Barberías" value={summary.businesses.total} detail={`${summary.businesses.active} activas`} tone="dark" />
        <MetricCard label="Usuarios" value={summary.users} detail="En todos los negocios" />
        <MetricCard label="Barberos" value={summary.barbers} detail="Perfiles registrados" />
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <Card className="overflow-hidden">
          <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-5 sm:px-6">
            <div>
              <h3 className="font-semibold text-zinc-950">Barberías recientes</h3>
          ...
          {recent.length === 0 ? (
            <p className="px-6 py-14 text-center text-sm text-zinc-500">Aún no hay barberías registradas.</p>
          ) : (
            <ul>
              {recent.map((business) => {

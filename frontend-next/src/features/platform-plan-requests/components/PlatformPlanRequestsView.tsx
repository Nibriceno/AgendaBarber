...
import { getPlatformPlanRequests, publishOnboarding, updatePlatformPlanRequestStatus } from "../api/platform-plan-requests.api";
...
export default function PlatformPlanRequestsView() {
  ...
  const publish = async (id: number) => {
    ...
    setNotice("");
    try {
      const published = await publishOnboarding(id);
      setResult((current) => current ? {
        ...current,
        items: current.items.map((item) => item.id === id ? {
          ...item,
          status: published.requestStatus,
          business: item.business ? {
            ...item.business,
            status: published.businessStatus,
            slug: published.businessSlug,
          } : item.business,
        } : item),
      } : current);
      setNotice(
        published.invitationEmailSent
          ? "Negocio publicado. La invitación fue enviada al administrador."
          : "Negocio publicado, pero el correo no pudo enviarse. Reenvía la invitación desde el detalle del negocio.",
      );
    } catch (requestError) {
      setError(getApiErrorMessage(requestError, "No fue posible publicar el negocio."));
    } finally {
      setUpdatingId(null);
    }
  };
      ...
      <div className="grid gap-4">
        {loading ? Array.from({ length: 3 }).map((_, index) => <Card key={index} className="h-44 animate-pulse bg-zinc-100" />) : result?.items.map((item) => (
          <Card key={item.id} className="p-5 sm:p-6">
            ...
            {item.business && <div className="mt-4 flex flex-col gap-3 rounded-xl border border-zinc-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-zinc-900">Tienda privada: {item.business.slug}</p><p className="mt-1 text-xs text-zinc-500">Revisa sus datos antes de publicarla. Sólo se habilita con el primer pago confirmado.</p></div><div className="flex flex-wrap gap-2"><Link href={`/super-admin/businesses/${item.business.id}`} className="inline-flex min-h-10 items-center justify-center rounded-xl border border-zinc-200 px-4 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">Revisar tienda</Link>{item.status === "PAID" && item.business.status === "PENDING" && <button type="button" disabled={updatingId === item.id} onClick={() => void publish(item.id)} className="inline-flex min-h-10 items-center justify-center rounded-xl bg-zinc-950 px-4 text-sm font-semibold text-white disabled:opacity-50">{updatingId === item.id ? "Publicando..." : "Publicar y enviar acceso"}</button>}</div></div>}
            ...
            {item.business?.subscriptions[0] && <div className="mt-4 rounded-xl border border-violet-100 bg-violet-50 px-4 py-3 text-sm text-violet-800"><span className="font-semibold">Suscripción mensual: {item.business.subscriptions[0].status}</span>{item.business.subscriptions[0].payments[0]?.paidAt ? ` · Primer pago ${new Date(item.business.subscriptions[0].payments[0].paidAt!).toLocaleString("es-CL")}` : " · Aún sin pago confirmado"}</div>}
